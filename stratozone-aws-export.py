...
parser.add_argument(
    "-n", "--no_perf", help="Do Not collect performance data.", action="store_true"
)
parser.add_argument(
    "-t",
    "--thread_limit",
...
def get_image_info(image_id, l_vm_instance):
    ...
    """
    try:
        disk_image = client.describe_images(
            ImageIds=[
                image_id,
            ]
        ).get("Images")
...
def get_image_size_details(instance_type, l_vm_instance):
    ...
    Returns:
        Dictionary object.
    """
    instance_type_info = client.describe_instance_types(
        InstanceTypes=[
            instance_type,
        ]
    ).get("InstanceTypes")
    ...
    l_vm_instance["AllocatedProcessorCoreCount"] = instance_type_info[0]["VCpuInfo"][
        "DefaultVCpus"
    ]
    return l_vm_instance
...
def report_writer(
    dictionary_data, field_name_list, file_name, directory="./output/vm/"
):
            ...
            if not args.no_public_ip:
                if "Association" in interface:
                    if len(interface["Association"]["PublicIp"]) > 0:
                        l_vm_instance["PublicIPAddress"] = interface["Association"][
                            "PublicIp"
                        ]
...
def get_metric_data_query(
    namespace, metric_name, dimension_name, dimension_value, unit, query_id=""
):
    ...
    try:
        ...
        perf_queries.append(
            get_metric_data_query(
                "AWS/EC2", "CPUUtilization", "InstanceId", vm_id, "Percent"
            )
        )
        perf_queries.append(
            get_metric_data_query("AWS/EC2", "NetworkOut", "InstanceId", vm_id, "Bytes")
        )
        perf_queries.append(
            get_metric_data_query("AWS/EC2", "NetworkIn", "InstanceId", vm_id, "Bytes")
        )
        for block_device in block_device_list:
            perf_queries.append(
                get_metric_data_query(
        ...
        if (
            len(response["MetricDataResults"][1]["Values"]) >= first_arr_size
            ...
            for i in range(0, first_arr_size):
                ...
                vm_perf_info["TimeStamp"] = get_formatted_datetime(
                    response["MetricDataResults"][0]["Timestamps"][i]
                )
...
if version(boto3.__version__) < version("1.20.20"):
    print(
        "You are using version of AWS Python SDK that is too old."
        "\nVersion installed: {}"
...
while run_script:
    ...
    logging.basicConfig(
        filename="./output/stratozone-aws-export.log",
        format=log_format,
        level=logging.ERROR,
    )
    ...
    if args.collection_mode == "VirtualMachine":
            ...
            for reservation in specific_instance["Reservations"]:
                for instance in reservation["Instances"]:
                    ...
                    if vm_instance["OsType"] == "unknown":
                        tmp_os_value = "Linux"
                        if (
                            "windows" in instance.get("PlatformDetails").lower()
                            or "sql" in instance.get("PlatformDetails").lower()
                        ):
                    ...
                    vm_instance = get_image_size_details(
                        instance.get("InstanceType"), vm_instance
                    )

                    if "Tags" in instance:
                        vm_instance = get_instance_tags(
                            instance.get("InstanceId"), instance["Tags"], vm_instance
                        )
                    else:
                    ...
                    if "NetworkInterfaces" in instance:
                        get_network_interface_info(
                            instance["NetworkInterfaces"], vm_instance
                        )
                    ...
                    vm_create_timestamp = get_disk_info(
                        instance["InstanceId"],
                        instance["BlockDeviceMappings"],
                        instance["RootDeviceName"],
                    )

                    vm_instance["CreateDate"] = get_formatted_datetime(
                        vm_create_timestamp
                    )
        ...
        print("\nExport Completed. \n")
        print(
            "vm-aws-import-files.zip generated successfully containing {} files.".format(
                created_files
            )
        )
        ...
        print("\nExport Completed. \n")
        print(
            "services-aws-import-files.zip generated successfully containing {} files.".format(
                created_files
            )
        )
        break
