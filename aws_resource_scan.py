...
not_available_strings = (
    reference_aws.not_available_for_region_strings
...
class RawListing(object):
    ...
    def add_to_dictionary(self):
        ...
        try:
            ...
            is_found, found_resource_data = self.find_dict_extract(
                container_name, self.response
            )
            item_count = 0
...
class FilteredListing(object):
    ...
    def __str__(self):
        opdesc = "{} {} {} ".format(
            self.input.service, self.input.region, self.input.operation
        )
        if len(self.resource_types) == 0 or self.resource_total_count == 0:
            return "{} (no resources found)".format(opdesc)
        return opdesc + ", ".join(
    ...
    def resources(self):
        ...
        complete = filter_aws.apply_filters(
            self.input, self.unfilter, response, complete
        )
        unfilter_list = self.unfilter

        # Special handling for service-level kms keys; derived from alias name.
        if (
            "kmsListKeys" not in unfilter_list
            and self.input.service == "kms"
            and self.input.operation == "ListKeys"
        ):
            try:
                aliases_file = "{}_{}_{}.json".format(
                    self.input.service, "ListAliases", self.input.region
                )
                ...
                service_key_ids = [
                    k.get("TargetKeyId")
                    for k in list_aliases.get("Aliases", [])
        ...
        if (
            "ec2InternetGateways" not in unfilter_list
            and self.input.service == "ec2"
            and self.input.operation == "DescribeInternetGateways"
        ):
            try:
                vpcs_file = "{}_{}_{}.json".format(
                    self.input.service, "DescribeVpcs", self.input.region
                )
...
def get_services():
    ...
    return [
        service
        for service in sorted(boto3.Session().get_available_services())
...
def get_listing_operations(service, region=None, selected_operations=()):
    ...
    for operation in sorted(client.meta.service_model.operation_names):
        ...
        required_members = (
            op_model.input_shape.required_members if op_model.input_shape else []
        )
    ...
    try:
        ...
        if listing_file.input.error == result_error:
            return ResultListing(
                listing, result_error, "Error(Error during processing of resources)"
            )
        if resource_count > 0:
            listing.add_to_dictionary()
            return ResultListing(
                listing, result_something, ", ".join(listing_file.resource_types)
            )
        else:
            return ResultListing(
                listing, result_nothing, ", ".join(listing_file.resource_types)
            )
        ...
        if verbose > 2:
            traceback.print_exc()
        result_type = (
            result_no_access if "AccessDeniedException" in str(exc) else result_error
        )
        if service == "ec2" and operation == "DescribeInstances":
            logging.error(exc)

        ignored_err = reference_aws.RESULT_IGNORE_ERRORS.get(service, {}).get(operation)
...
def execute_query(to_run, verbose, parallel, results_by_type):
    ...
    with contextlib.closing(ThreadPool(parallel)) as pool:
        for result in pool.imap_unordered(
            functools.partial(acquire_listing, verbose), to_run
        ):
...
def do_query(
    services, selected_regions=(), selected_operations=(), verbose=0, parallel=32
):
    ...
    for service in services:
        for region in get_regions_for_service(service, selected_regions):
            for operation in get_listing_operations(
                service, region, selected_operations
            ):
                if operation in reference_aws.DEPENDENT_OPERATIONS:
                    dependencies[
                        reference_aws.DEPENDENT_OPERATIONS[operation], region
                    ] = [service, region, reference_aws.DEPENDENT_OPERATIONS[operation]]
    ...
    results_by_type = execute_query(
        dependencies.values(), verbose, parallel, results_by_type
    )
...
def scan_aws(service_collection, region_list, thread_limit):
    ...
    while run_script:
        ...
        for region in region_list:
            ...
            regions = [region]
            do_query(services, regions, operation, verbose=0, parallel=thread_limit)
            _clients = {}
